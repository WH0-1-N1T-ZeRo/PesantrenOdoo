...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move,
        ...
        if target_move == 'posted':
            move_state = ['posted']
        arg_tree = (tuple(move_state), tuple(account_type))
        ...
        if reconciled_after_date:
            ...
            arg_tree += (tuple(reconciled_after_date),)
        arg_tree += (date_from, tuple(company_ids))
        query = '''
        ...
        for i in range(5):
            ...
            if periods[str(i)]['start'] and periods[str(i)]['stop']:
                ...
                dates_query += ' <= %s)'
                args_tree += (periods[str(i)]['stop'],)
            args_tree += (date_from, tuple(company_ids))
            query = '''SELECT l.id
                    FROM account_move_line AS l, account_account, account_move am
                    WHERE (l.account_id = account_account.id) AND (l.move_id = am.id)
                        AND (am.state IN %s)
