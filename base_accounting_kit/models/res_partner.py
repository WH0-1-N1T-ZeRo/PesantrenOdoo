...
class ResPartner(models.Model):
    ...
    invoice_tree = fields.One2many('account.move', 'partner_id',
    ...
    def get_min_date(self):
        ...
        today = date.today()
        for this in self:
            if this.invoice_tree:
                min_tree = this.invoice_tree.mapped('invoice_date_due')
                while False in min_tree:
                    min_tree.remove(False)
                return min(min_tree)
            else:
                return today
